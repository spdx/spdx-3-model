...
Additional information for an artifact when a standard property is unavailable.

## Description

Provides additional information about an artifact.
It is structured as a key-value pair and used to extend the artifact's
...
the current SPDX 3 specification.
...
pair structure cannot fully capture, an extension based on the Extension
profile can be considered.

*Difference from `comment` property:*

The `additionalInformation` property represents an intrinsic characteristic of
the artifact itself, whereas the `comment` property provides supplementary
remarks or descriptions originating from an external entity.
