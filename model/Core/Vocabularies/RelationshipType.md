...
- hasConcludedLicense: The `from` SoftwareArtifact or AbstractPackage is concluded by the SPDX data creator to be governed by each `to` AnyLicenseInfo.
...
- hasDeclaredLicense: The `from` SoftwareArtifact or AbstractPackage was discovered to actually contain each `to` AnyLicenseInfo (for example, as detected by automated tooling).
...
- hasInstance: The `from` AbstractPackage has more specific instances that are the `to` Element(s).
